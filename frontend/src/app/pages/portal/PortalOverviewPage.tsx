...
export default function PortalOverviewPage() {
  ...
  useEffect(() => {
    ...
    const loadDashboard = async () => {
      try {
        ...
        const [shipmentData, requestData, notificationData] =
          await Promise.all([
            shipmentsService.listShipmentsForCustomer(user.id),
            requestsService.listRequestsForCustomer(user.id),
            notificationsService.listNotifications(user.id),
          ]);

        setShipments(shipmentData);
        setRequests(requestData);
        setActivity(notificationData);
      } catch (err) {
  ...
  if (error) {
    return (
      <div className="p-6 lg:p-10">
        ...
        <div className="mt-6 rounded-sm border border-danger/30 bg-danger/5 p-6 text-sm text-danger">
          {error}
        </div>
      </div>
    );
  ...
  const pendingRequests = requests.filter((r) =>
    ['pending', 'under_review', 'quote_provided'].includes(r.status),
  );
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <div className="flex items-center justify-between">
            ...
            <Link
              to="/portal/shipments"
              className="flex items-center gap-1 text-sm font-medium text-cargo hover:text-cargo-dark"
            >
          ...
          <div className="mt-4 overflow-hidden rounded-sm border border-line bg-white">
            {recentShipments.length === 0 ? (
              <p className="p-6 text-center text-[15px] text-ink-mid">
                No shipments yet.
              </p>
            ) : (
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <p className="mt-0.5 text-xs text-ink-mid">
                          Est. delivery {formatDate(s.estimatedDelivery)}
                        </p>
                      </div>

                      <StatusBadge
                        label={shipmentStatusLabels[s.status]}
